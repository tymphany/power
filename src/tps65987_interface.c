...
int tps65987_get_lvp15(void)
{
    ...
    if(memcmp(buf, PDO_20V_2d25A, 4) == 0)
    {
        printf("ActiveContractPDO is 20V/2.25A\n\n");
        return 0;
    }
...
int tps65987_get_PDO_status(unsigned char *current)
{
    Active_Contract_PDO buf[60] = {0};
    if(tps65987_i2c_read(I2C_ADDR, REG_ActiveContractPDO, buf, 4) != 0)
    {
        printf("\nget ActiveContractPDO err \n");
        return -1;
    }

    current[0] = (buf->Max_Current)/5;
    printf("Max current is %d0mA\n",buf->Max_Current);
    printf("Voltage is %dV\n",(buf->Voltage)/20);
    printf("Max current is 0x%02x\n",(buf->Max_Current)/5);
    return 0;
}
